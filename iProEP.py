    ...
    for eachline in f:
        if eachline[0] == '>':
            ...
            if len(seq) >= pretypelength:
                ...
                print("The length of the %s is less than %s-bp, it has no biological meaning for promoter."%(annotation,pretypelength))
    g.close()
    f.close()
...
def runSVM(pathPrefix):
    ...
    if 'linux' in sys.platform:
        commands.append(r"%slibsvm-3.22/svm-scale -r scale.rule optimalFea.txt > seq.scale"%pathPrefix)
        commands.append(r"%slibsvm-3.22/svm-predict -b 1 seq.scale predict.model res.txt >out.txt"%pathPrefix)
    if 'win' in sys.platform:
        commands.append(r"%ssvm-scale.exe -r scale.rule optimalFea.txt > seq.scale"%pathPrefix)
        commands.append(r"%ssvm-predict.exe -b 1 seq.scale predict.model res.txt >out.txt"%pathPrefix)
    ...
    for eachline in f:
        ...
        if temp[0]=='labels':
            ...
            if(temp[0]=='1'):
                ...
                g1.write('%s is predicted as Promoter. Its probability score of being a promoter is %s.\n'%(annotation[countn-1],temp[1]))
            if(temp[0]=='2'):
                g.write('%s\t%s\tNo\t%s\n'%(countn,annotation[countn-1],temp[1]))
    g.close()
    f.close()
...
if __name__=='__main__':
    ...
    filelist = ['pse&pcsfFea.txt','pseFea.txt','pcsfFea.txt','optimalFea.txt','seq.scale','out.txt']
    for each in filelist:
        os.remove(each)
    print("Prediction Finished!\n***There are %s promoter in the query sequences.***\nFind All_Result.csv and Promoter.txt get the results."%countp)
